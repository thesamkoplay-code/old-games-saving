...
const PORT = Number(process.env.PORT || 2964);
...
function setSessionCookie(res, sid) {
  res.setHeader(
    "Set-Cookie",
    `${COOKIE_NAME}=${encodeURIComponent(sid)}; HttpOnly; SameSite=Lax; Path=/; Max-Age=86400`
  );
}
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === "POST" && pathname === "/api/login") {
    ...
    sessions.set(sid, { admin: true, expiresAt: Date.now() + 24 * 60 * 60 * 1000 });
