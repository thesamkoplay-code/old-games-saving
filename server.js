...
async function storeOptionalImage(files, fieldName, category) {
  ...
  if (!file.contentType.toLowerCase().startsWith("image/")) {
    throw new Error("Cover and gallery files must be images.");
  }
...
async function storeImageList(files, fieldName, category) {
  ...
  for (const file of list) {
    if (!file.contentType.toLowerCase().startsWith("image/")) {
      throw new Error("The gallery accepts image files only.");
    }
...
function parseExternalLinkLine(line, fallbackName) {
  ...
  if (!url) {
    throw new Error(`Invalid link: ${raw}`);
  }
...
function externalImageListFromField(fields, fieldName, category) {
  ...
  return raw
    .split(/\r?\n/)
    .map((line) => parseExternalLinkLine(line, "image"))
    .filter(Boolean)
...
function externalFileListFromField(fields, fieldName, category) {
  ...
  return raw
    .split(/\r?\n/)
    .map((line) => parseExternalLinkLine(line, "file"))
    .filter(Boolean)
...
function requireAdmin(req, res) {
  ...
  sendJson(res, 401, { error: "Sign in with the admin code." });
  return false;
}
...
async function handleCreateItem(req, res) {
  ...
  if (!title || !description) {
    return sendJson(res, 400, { error: "Title and download instructions are required." });
  }
...
async function handleUpdateItem(req, res, itemId) {
  ...
  if (!item) return sendJson(res, 404, { error: "Item does not exist." });
...
async function handleDeleteItem(req, res, itemId) {
  ...
  if (index === -1) return sendJson(res, 404, { error: "Item does not exist." });
...
async function handleAddComment(req, res, itemId) {
  ...
  const author = safeText(data.author, 40) || "Anonymous";
...
async function handleDeleteComment(req, res, commentId) {
  ...
  if (store.comments.length === before) {
    return sendJson(res, 404, { error: "Comment does not exist." });
  }
  await saveStore(store);
  sendJson(res, 200, { ok: true });
}
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === "POST" && pathname === "/api/login") {
    ...
    if (!verifyAdminCode(data.code, admin)) {
      return sendJson(res, 403, { error: "Wrong admin code." });
    }
...
if (require.main === module) {
  ensureStorageOnce()
    ...
    http.createServer(handleRequest).listen(PORT, () => {
      console.log(`Old Games Saving is running at http://localhost:${PORT}`);
    });
    })
    .catch((error) => {
    console.error("Could not prepare storage:", error);
    process.exit(1);
    });
}
