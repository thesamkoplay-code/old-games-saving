...
function fileSize(bytes) {
  if (bytes === null || bytes === undefined) return "Externí odkaz";
...
els.gameForm.addEventListener("submit", async (event) => {
  ...
  if (location.hostname.endsWith("vercel.app") && hasSelectedLocalFiles()) {
    showToast("Na Vercelu nahraj soubor jinam a sem vlož odkaz.");
    return;
  }
